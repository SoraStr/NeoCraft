...
#[test]
fn test_read_simple_properties() {
    ...
    let props = files::read_properties(&path).unwrap();
...
#[test]
fn test_read_properties_with_comments_and_blanks() {
    let dir = tempfile::TempDir::new().unwrap();
    let path = write_fixture(dir.path(), "server.properties",
        "# Top comment\n\
         \n\
...
#[test]
fn test_read_properties_with_colon_separator() {
    let dir = tempfile::TempDir::new().unwrap();
    let path = write_fixture(dir.path(), "server.properties",
        "server-port:25565\nmotd:Test\n"
    );
...
#[test]
fn test_read_properties_trims_whitespace() {
    let dir = tempfile::TempDir::new().unwrap();
    let path = write_fixture(dir.path(), "server.properties",
        "  server-port = 25565  \n  motd =  Padded MOTD  \n"
    );
...
#[test]
fn test_write_properties_handles_new_keys() {
    let dir = tempfile::TempDir::new().unwrap();
    let path = dir.path().join("out.properties");
...
fn test_read_nonexistent_file_errors() {
    let result = files::read_properties(Path::new("/nonexistent/path/server.properties"));
